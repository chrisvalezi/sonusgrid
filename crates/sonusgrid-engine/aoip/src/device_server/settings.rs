...
fn create_self_info(
  ...
  let mut result = DeviceInfo {
    ...
    board_name: "SonusGrid".to_owned(),
    manufacturer: "SonusGrid".to_owned(),
...
impl Settings {
  ...
  pub fn make_rx_channels(&mut self, count: usize) {
    // SonusGrid patch: friendly_name is the numeric ID without an RX/TX
    // prefix so the Dante Controller matrix shows clean "01", "02", … on
    // both axes. Direction is implied by the column/row position; the
    // prefix was redundant and visually noisy.
    self.self_info.rx_channels = (1..=count)
      .map(|id| Channel {
        factory_name: format!("{id:02}"),
        friendly_name: Arc::new(RwLock::new(format!("{id:02}"))),
      })
      .collect();
  }
