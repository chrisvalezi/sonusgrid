	...
	:
        ...
        if test x"\$DISPLAY" != x -a x"\$xterm_loop" = x; then  # No, but do we have X?
            if xset q > /dev/null 2>&1; then # Check for valid DISPLAY variable
                # SonusGrid: Debian alternative first, then modern GNOME/KDE/Xfce terminals.
                GUESS_XTERMS="x-terminal-emulator gnome-terminal konsole xfce4-terminal mate-terminal kgx ptyxis tilix alacritty kitty foot lxterminal xterm rxvt dtterm eterm Eterm kvt aterm terminology"
                ...
                # SonusGrid: modern terminals (kgx, ptyxis, gnome-terminal) treat
                # a quoted "-e 'cmd args'" as ONE program name. Hand every
                # terminal a single token: a throwaway wrapper script.
                if test \`echo "\$0" | cut -c1\` = "/"; then abs0="\$0"; else abs0="\$PWD/\$0"; fi
                xwrap=\`mktemp "\${TMPDIR:-/tmp}/makeself-xwin.XXXXXX"\`
                printf '#!/bin/sh\ncd "%s"\nexec "%s" --xwin %s\nrm -f "%s"\n' "\$PWD" "\$abs0" "\$initargs" "\$xwrap" > "\$xwrap"
                chmod 755 "\$xwrap"
                case "\$XTERM" in
                    gnome-terminal|kgx|ptyxis|tilix|mate-terminal) exec \$XTERM -- "\$xwrap" ;;
                    *) exec \$XTERM -e "\$xwrap" ;;
                esac
            fi
        fi
